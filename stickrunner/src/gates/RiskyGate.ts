...
export class RiskyGate extends BaseGate {
  ...
  protected createGateGeometry(): void {
    const postGeometry = new THREE.BoxGeometry(0.35, 3.2, 0.35);
    ...
    for (let i = 0; i < segments; i++) {
      const segmentColor = i % 2 === 0 ? this.color : (this.color === CONFIG.COLORS.POSITIVE_GATE ? 0xffaa00 : 0x660000);
      ...
      const yPos = 0.5 - (segmentHeight * segments / 2) + (i * segmentHeight) + (segmentHeight / 2);
